  ...
  if (!req.params.email) {
      ...
      User.findOne({ email: req.params.email }, (err, user) => {
        if (err) {
          ...
          // Check if user's email is taken
          if (user) {
            res.json({ success: false, message: 'Email is already taken' }); // Return as taken email
          } else {
            res.json({ success: true, message: 'Email is available' }); // Return as available email
          }
        }
      });
  }
...
router.get('/checkUsername/:username', (req, res) => {
  ...
  if (!req.params.username) {
      ...
      User.findOne({ email: req.params.username }, (err, user) => { // Check if connection error was found
        if (err) {
          res.json({ success: false, message: err }); // Return connection error
        } else {
