            ...
            if (err) {
              ...
              if (!user) {
                ...
                if (user.username !== blog.createdBy) {
                  res.json({ success: false, message: 'You are not authorized to edit this blog.' }); // Return authentication error
                } else {
          ...
          User.findOne({ _id: req.decoded.userId }, (err, user) => {
            ...
            if (err) {
              ...
              if (!user) {
                ...
                // Check if user logged in the one requesting to update blog post
                if (user.username !== blog.createdBy) {
          ...
          User.findOne({ _id: req.decoded.userId }, (err, user) => {
            ...
            if (err) {
              ...
              if (!user) {
                ...
                // Check if user who liked the post has already liked the blog post before
                if (blog.likedBy.includes(user.username)) {
                  ...
                  if (blog.dislikedBy.includes(user.username)) {
                    ...
                    blog.dislikedBy.splice(arrayIndex, 1); // Remove user from array of likedBy array
                    // Save blog post data
                    blog.save((err) => {
                      // Check if error was found
                      if (err) {
            ...
            User.findOne({ _id: req.decoded.userId }, (err, user) => {
              ...
              if (err) {
                ...
                if (!user) {
                  ...
                  // Check if user who disliked post is the same person who originated the blog post
                  if (user.username === blog.createdBy) {
...
router.post('/comment', (req, res) => {
  // Check if comment was provided in request today
  if (!req.body.comment) {
